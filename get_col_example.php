...
<body>
    ...
    <div class="form-group">
        <label for="value">搜尋值（選填）：</label>
        <input type="text" id="value" placeholder="請輸入要搜尋的值（可留空）">
    </div>
    ...
    <script>
        async function fetchData() {
            ...
            if (!tableName || !columnName) {
                resultDiv.innerHTML = '<p class="error">請填寫資料表名稱和欄位名稱</p>';
                return;
            }
                ...
                if (data.status === 'success') {
                    ...
                    if (value) {
                        filteredData = data.data.filter(row =>
                            String(row[columnName]).toLowerCase().includes(value.toLowerCase())
                        );
                    }
                    ...
                    tableHtml += '</table>';
                    tableHtml += `<p class="count">共找到 ${filteredData.length} 筆資料</p>`;
